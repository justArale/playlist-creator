// import "./Footer.css";

function Footer() {
  return (
    <footer className="inset-x-0 bottom-0 mb-2 w-full justify-center bg-white px-3 py-2 text-center text-sm text-gray-900 md:mb-10 md:pb-6 md:text-lg lg:text-xl ">
      {/* <div className="w-full bg-white text-gray-900 text-center justify-center"> */}
      <a href="https://github.com/justArale/playlist-creator">
        playlist-creator{" | "}
      </a>{" "}
      made with ❤️ {"   "} and{"  "} 🎵 by{"  "}
      <a href="https://github.com/annagy07">annagy07</a>
      {" | "}
      <a href="https://github.com/camialbuq">Cami Albuquerque</a>
      {" | "}
      <a href="https://github.com/justArale">Arale</a>
      {/* </div> */}
    </footer>
  );
}

export default Footer;
